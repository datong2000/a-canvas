...
export function Rotation(angle: number, v: XYZ): Matrix {
    let sina: number = sin(angle), cosa: number = cos(angle), mcos: number = 1 - cosa;
    return new Matrix([
...
export default class Matrix {
    ...
    mul(m: Matrix): Matrix {
        let v: number[] = [], i: number, j: number;
        for (i = 1; i <= 3; ++i)
            for (j = 1; j <= 3; ++j) {
    ...
    xform(v: XYZ): XYZ {
        let value: XYZ = { x: null, y: null, z: null }, x = v.x, y = v.y, z = v.z;
